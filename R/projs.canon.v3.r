...
"projs.canon" <- function(formulae, orthogonalize = "differencingfirst", 
...
{ #examine the relationships between the sets of mutually orthogonal projection matrices for the supplied formulae
  ...
  Q[[1]] <- projs.structure(formulae[[1]], orthogonalize = orthog[1], data=data, ...)
  ...
  for (k in 1:ncomb)
  { ktier <- k + 1
    Q[[ktier]] <- projs.structure(formulae[[ktier]], orthogonalize = orthog[ktier], data=data, ...)
...
"summary.pcanon" <- function(object, which.criteria = c("aefficiency", "eefficiency"), ...)
