...
{ #generate a set of mutually orthogonal projection matrices, one for each term in the formula
  ...
  if (length(terms) > 1)
  { if (orthogonalize == "differencingfirst") #by difference
    { orthogonal <- TRUE
...
{ daeTolerance <- get("daeTolerance", envir=daeEnv)
  criteria <- vector(mode="list", length = 5)
  names(criteria) <- c('aefficiency','mefficiency','sefficiency','eefficiency','order')
...
{ if (!is.list(Q1) | !is.list(Q2))
  ...
  criteria <- c('aefficiency','mefficiency','sefficiency','eefficiency','order')
...
"summary.p2canon" <- function(object, which.criteria = c("aefficiency", "eefficiency"), ...)
...
{ if (!inherits(object, "p2canon"))
  ...
  criteria <- c("aefficiency", "eefficiency", "mefficiency", "sefficiency", "order")
